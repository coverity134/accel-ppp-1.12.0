...
int rad_packet_recv(int fd, struct rad_packet_t **p, struct sockaddr_in *addr)
{
	...
	*p = NULL;
	...
	while (1) {
		if (addr)
			n = recvfrom(fd, pack->buf, REQ_LENGTH_MAX, 0, addr, &addr_len);//SLIVER_SOURCE 读数据到pack->buf同时更新ptr
		else
			n = read(fd, pack->buf, REQ_LENGTH_MAX);//SLIVER_SOURCE 读数据到pack->buf同时更新ptr
		if (n < 0) {
		...
		if (da) {
			...
			if (!da->array) {
				switch (da->type) {
					...
					case ATTR_TYPE_IFID:
					case ATTR_TYPE_IPV6ADDR:
						memcpy(&attr->val.integer, ptr, len);//SLIVER_SINK len
						break;
					case ATTR_TYPE_IPV6PREFIX:
